...
public class TextFieldController implements Lexer.LexCallback {
    ...
    public void onPrintableChar(char c) {
        ...
        //Toast.makeText(mTextField.getContext(), c, 0).show();
        ...
        switch (c) {
            case Language.BACKSPACE:
                //Toast.makeText(mTextField.getContext(), "backspace",0).show();
                if (selectionDeleted) {
                    break;
                }
    ...
    void replaceComposingText(int from, int charCount, String text) {
        ...
        //Toast.makeText(mTextField.getContext(),"called "+from+" "+ charCount+" "+text.length(),0).show();
        if(text.length() == 1&& text.equals("\n")&&!_isInSelectionMode){
            text = new String(createAutoIndent());
            charCount= 0;
            //return;
        }
        int invalidateStartRow, originalOffset;
        ...
        //delete selection
        if (_isInSelectionMode) {
            //from and charcount is used for alternaye implementation.
            from = mTextField.mSelectionAnchor;
            charCount = mTextField.mSelectionEdge - mTextField.mSelectionAnchor;
            int delFromRow = mTextField.hDoc.findRowNumber(mTextField.mSelectionAnchor);
            invalidateStartRow = delFromRow;
            originalOffset = mTextField.hDoc.getRowOffset(invalidateStartRow);
